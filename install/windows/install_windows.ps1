...
& $Python[0] @($Python[1..($Python.Length - 1)]) -c "import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)"
...
if (Test-Path (Join-Path $ProjectRoot "requirements.txt")) {
    Copy-Item -Force (Join-Path $ProjectRoot "requirements.txt") (Join-Path $InstallDir "requirements.txt")
}
if (Test-Path (Join-Path $ProjectRoot "README.md")) {
    Copy-Item -Force (Join-Path $ProjectRoot "README.md") (Join-Path $InstallDir "README.md")
}
...
if ((Test-Path $SourceConfig) -and !(Test-Path $ConfigPath)) {
    ...
    & $Python[0] @($Python[1..($Python.Length - 1)]) (Join-Path $InstallDir "forgrequest.py") --init-config -c $ConfigPath
}

& $Python[0] @($Python[1..($Python.Length - 1)]) -c "import requests" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "[+] Installing requests dependency for the current user..."
    $ReqPath = Join-Path $InstallDir "requirements.txt"
    if (Test-Path $ReqPath) {
        & $Python[0] @($Python[1..($Python.Length - 1)]) -m pip install --user -r $ReqPath
    } else {
        & $Python[0] @($Python[1..($Python.Length - 1)]) -m pip install --user requests
    }
}

$PyPrefix = ($Python -join " ")
$Wrapper = @"
@echo off
